	...
	parser_i.on('m3u', function(m3u)
	{
		...
		fs.writeFile(new_manifest, str, function(err) {
			if(err) {
				...
				console.log("---------------------");
				callback(null, mediaFile, clippoint, splitfile_1, splitfile_2);
			}
		});
	} );
}
...
function cutMediaFile(res) {
	...
	return function (err, fileToCut, whereToCut, cut1, cut2) {
		...
		console.log(fileToCut, whereToCut, cut1, cut2, callback);
		console.log("----------------------------------------");
		var ff = new ffmpeg(fileToCut);
		ff.then(function (video) {
			video.addCommand('-ss', '0', '-to', whereToCut, '-c', 'copy' );
			video.save(cut1, function (error, file) {
				if (error) {
					return callback(error);
				}
				ff.then(function (video) {
					video.addCommand('-ss', whereToCut, '-c', 'copy' );
					video.save(cut2, callback );
				});
			});
		});
	};
